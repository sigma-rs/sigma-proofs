...
fn zero_scalar_witness_discrete_log() {
    ...
    let nizk = NISigmaProtocol::<SchnorrProof<G>, ShakeCodec<G>>::new(
        b"test-fiat-shamir-zero-witness",
        protocol,
    );

    let result_batch = nizk.prove_batchable(&witness, &mut rng);
    let result_compact = nizk.prove_compact(&witness, &mut rng);

    assert!(
        result_batch.is_err(),
        "Expected error when generating batchable proof with zero witness"
    );
    assert!(
        result_compact.is_err(),
        "Expected error when generating compact proof with zero witness"
    );
}
...
fn zero_generator_discrete_log() {
    ...
    assert!(
        result_batch.is_err(),
        "Expected error when generating batchable proof with zero generator"
    );
    assert!(
        result_compact.is_err(),
        "Expected error when generating compact proof with zero generator"
    );
}
...
fn fully_zero_pedersen_commitment() {
    ...
    let protocol = SchnorrProof::from(morphismp);
    let nizk = NISigmaProtocol::<SchnorrProof<G>, ShakeCodec<G>>::new(
        b"test-fiat-shamir-pedersen-commitment",
        protocol,
    );
    ...
    assert!(
        result_batch.is_err(),
        "Expected error when generating batchable proof for fully zero Pedersen commitment"
    );
    assert!(
        result_compact.is_err(),
        "Expected error when generating compact proof for fully zero Pedersen commitment"
    );
}
...
fn trivial_relation() {
    ...
    let (morphismp, witness) = {
        ...
        cs.append_equation(out_var, LinearCombination::from(Vec::<Term<G>>::new()));
