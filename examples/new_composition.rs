...
// TODO: Get this compiling again, or scrap it and start over.
#[cfg(false)]
mod example {
    ...
    use sigma_proofs::{
        ...
        composition::new::Or,
        errors::Error,
        linear_relation::{Allocator, CanonicalLinearRelation, ScalarMap, ScalarVar},
        LinearRelation, Nizk,
    };
    ...
    fn create_relation(
        ...
        Q: G,
        H: G,
    ) -> (Or<CanonicalLinearRelation<G>>, ScalarVar<G>, ScalarVar<G>) {
        ...
        (
            Or(vec![rel1.canonical().unwrap(), rel2.canonical().unwrap()]),
            x1,
            x2,
        )
    }
    ...
    fn prove(P1: G, x2: Scalar, H: G) -> ProofResult<Vec<u8>> {
        ...
        let witness = vec![
            ScalarMap::from([(var_x1, Scalar::from(0u64))]),
            ScalarMap::from([(var_x2, x2)]),
        ];
